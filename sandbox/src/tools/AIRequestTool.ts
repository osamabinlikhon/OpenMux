...
import XRequestFunction, { XRequestClass, XRequestOptions, XRequestCallbacks } from '../utils/XRequest';
...
export class AIRequestTool implements Tool {
  name = 'ai-request';
  description = 'Make requests to external AI APIs (MiniMax, GLM, Kimi, etc.) with advanced retry and streaming support';
  ...
  private _initializeClients(): void {
    ...
    uniqueBaseURLs.forEach((baseURL) => {
      const callbacks: XRequestCallbacks = {
        onSuccess: (chunks) => {
  ...
  async streamChat(params: {
    ...
    max_tokens?: number;
    streamId?: string;
  }): Promise<{ success: boolean; streamId: string; message?: string; error?: string }> {
    try {
      const model = AI_MODELS[params.modelId];
      ...
      await client.post('/chat/completions', requestData as any);
   ...
   */
  async abortStream(streamId: string): Promise<{ success: boolean; message: string }> {
    try {
